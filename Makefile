...
#  の責任を負わない．
#
#  $Id: Makefile 735 2016-04-03 14:11:07Z ertl-hiro $
#
...
#  mrubyのアプリケーションファイルのディレクトリ
MRUBY_APP_DIR = $(SRCDIR)/workspace/mruby_app

#  mrubyのクラスファイルのディレクトリ
...
#  mruby 関係の設定
MRUBY_SRC_DIR = $(SRCDIR)/../mruby-1.3.0
MRUBY_LIB = -lmruby #$(MRUBY_SRC_DIR)/build/ARM/lib/libmruby.a
LIBS := $(MRUBY_LIB) $(LIBS)
...
#  TLSF 関係の設定
TLSF_SRC_DIR = $(SRCDIR)/../TLSF-2.4.6/src

#  FatFs 関係の設定
FATFS_SRC_DIR = $(SRCDIR)/../FatFs/
...
TECSGEN = ruby -Eutf-8 $(SRCDIR)/tecsgen/tecsgen.rb -L.
...
INCLUDES := -I. -I$(SRCDIR)/include $(INCLUDES) -I$(SRCDIR) \
      		...
      		-I$(TLSF_SRC_DIR) \
      		-I$(FATFS_SRC_DIR) \
      		-I$(TECSLIB_DIR)/gr-peach -I$(TECSLIB_DIR)/common \
 			-I$(SRCDIR)/tecsgen/tecs/mruby \
			-I$(SRCDIR)/tecsgen/tecs/TLSFMalloc \
            -I$(SRCDIR)/tecsgen/tecs/posix
LDFLAGS := $(LDFLAGS) -L $(MRUBY_SRC_DIR)/build/ARM/lib/ -Wl,-Map=$(OBJNAME).map,--cref
LIBS := $(LIBS) $(CXXLIBS)
...
APPL_DIRS := $(APPLDIRS) $(SRCDIR)/library $(TECSLIB_DIR)/gr-peach $(TECSLIB_DIR)/common
...
APPL_COBJS := $(APPL_COBJS) log_output.o vasyslog.o t_perror.o strerror.o tlsf.o \
				ffarch.o time.o diskio.o ff.o sdfs.o ccsbcs.o
...
#  コンフィギュレータに関する定義
#
CFG_TABS := --api-table $(SRCDIR)/kernel/kernel_api.def:kernel \
...
#
vpath %.c $(KERNEL_DIRS) $(SYSSVC_DIRS) $(APPL_DIRS) $(SRCDIR)/tecsgen/tecs/mruby $(TLSF_SRC_DIR) $(FATFS_SRC_DIR)
...
kernel_cfg.timestamp: cfg1_out.db cfg1_out.syms cfg1_out.srec
	$(CFG) --pass 2 --kernel asp $(INCLUDES) -T $(TARGET_KERNEL_TRB):kernel
